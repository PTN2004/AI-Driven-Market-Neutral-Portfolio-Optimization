...
class BacktestVisualizer:
    ...
    def plot_equity_curves(self, results_df: pd.DataFrame):
        ...
        plt.plot(dates, cum_ai, label='AI Market Neutral', color='#2b5c8f', linewidth=2.5)
        plt.plot(dates, cum_bm, label='VN-Index (Buy & Hold)', color='#d95f02', linewidth=1.8, linestyle='--')
    ...
    def plot_drawdowns(self, results_df: pd.DataFrame):
        ...
        plt.plot(dates, dd_ai, label='AI Market Neutral MDD', color='#2b5c8f', linewidth=2)
        plt.plot(dates, dd_bm, label='VN-Index MDD', color='#d95f02', linewidth=1.5, alpha=0.7)
    ...
    def plot_rolling_beta(self, results_df: pd.DataFrame):
        ...
        if 'portfolio_beta' in results_df.columns:
            beta_series = results_df['portfolio_beta'].rolling(window=10, min_periods=1).mean()
        else:
            beta_series = pd.Series(np.zeros(len(dates)))

        plt.plot(dates, beta_series, color='#2ca02c', linewidth=2, label='10-Day Rolling Portfolio Beta')
        plt.axhline(0.0, color='red', linestyle='--', alpha=0.7, label='Market Neutral Target (Beta = 0)')
        plt.axhspan(-0.05, 0.05, color='green', alpha=0.1, label='Neutrality Tolerance Band (±0.05)')
    ...
    def plot_exposure_history(self, results_df: pd.DataFrame):
        ...
        gross = results_df.get('gross_exposure', pd.Series(1.0, index=results_df.index)) * 100
        net = results_df.get('net_exposure', pd.Series(0.0, index=results_df.index)) * 100

        plt.plot(dates, gross, label='Gross Exposure (%)', color='#9467bd', linewidth=2)
        plt.plot(dates, net, label='Net Exposure (%)', color='#1f77b4', linewidth=2, linestyle='-.')
